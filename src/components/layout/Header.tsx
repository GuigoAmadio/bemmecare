...
export default function Header() {
  ...
  // Detectar scroll para esconder/mostrar banner
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <>
      {/* Oferta do Dia - Faixa Superior (desliza para cima quando scrolla) */}
      <div
        className={`bg-[#fff5e7] w-full relative z-50 opacity-95 transition-all duration-500 ease-in-out ${
        ...
        <div className="container mx-auto px-6 py-2">
          <div className="flex items-center justify-center text-sm">
            <div className="flex items-center gap-2 text-primary font-semibold">
              ...
              <span>Oferta do Dia: 20% OFF em todos os óleos essenciais</span>
              <span className="animate-pulse">🔥</span>
            </div>
          </div>
        </div>
      ...
      <header
        className={`sticky top-0 z-40 bg-[#fff5e7] opacity-95 backdrop-blur-sm border-b border-border shadow-sm transition-all duration-500 ease-in-out ${
          ...
          <div className="flex items-center justify-between py-4">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              <Link href="/" className="flex items-center group">
                <span className="text-3xl font-bold text-primary group-hover:scale-105 transition-transform">
                  BemmeCare
                </span>
              </Link>
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-sm text-text-primary hover:text-primary transition-colors duration-200 font-medium relative group"
                >
                  {item.name}
            ...
            <div className="flex items-center gap-3">
              <div className="hidden lg:flex flex-1 justify-end max-w-sm">
                <div className="relative w-full">
                  <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 h-5 w-5" />
                  <input
                    type="text"
                    placeholder="Buscar produtos..."
                    className="w-full pl-12 pr-4 py-2 text-sm border-2 border-border rounded-xl bg-background/50 backdrop-blur-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all duration-300"
                  />
                </div>
              </div>
              {/* Mobile Search */}
              ...
              {isAuthenticated ? (
                ...
                <div className="hidden sm:flex items-center gap-3">
                  <Link
                    href="/auth/login"
                    className="px-4 py-2 font-medium text-primary transition-all duration-300 rounded-lg hover:bg-[#8b966c] hover:text-white hover:scale-105"
                  >
                    Entrar
                  </Link>
                  <Link
